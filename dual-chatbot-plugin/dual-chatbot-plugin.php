 ...
 * Description: Provides a dual chatbot for FAQ and member advisory within WordPress. Implements serverâ€‘side RAG retrieval and integration with OpenAI for conversational support.
 * Version:     1.0.0
...
// Ensure textdomain is loaded early and exactly from /languages
add_action('plugins_loaded', function () {
    load_plugin_textdomain('dual-chatbot', false, dirname(plugin_basename(__FILE__)) . '/languages');
});

// i18n fallback: if de_AT MO is missing, fall back to de_DE for this plugin only.
add_filter('load_textdomain_mofile', function ($mofile, $domain) {
    if ($domain !== 'dual-chatbot') {
        return $mofile;
    }
    ...
    $looksLikeDeAT = (stripos((string)$locale, 'de_AT') === 0) || str_ends_with((string)$mofile, 'de_AT.mo');
    if ($looksLikeDeAT) {
        $fallback = plugin_dir_path(__FILE__) . 'languages/dual-chatbot-de_DE.mo';
...
// Ensure German labels render for de_* locales by force-loading de_DE.mo when needed.
add_action('admin_init', function () {
    $locale = function_exists('get_user_locale') ? get_user_locale() : get_locale();
    if (stripos((string)$locale, 'de_') !== 0) {
        return;
    }
    $probe = translate_with_gettext_context('Data & Privacy', 'settings tab label', 'dual-chatbot');
    if ($probe === 'Data & Privacy') {
        $fallback = plugin_dir_path(__FILE__) . 'languages/dual-chatbot-de_DE.mo';
        if (is_readable($fallback)) {
            unload_textdomain('dual-chatbot');
            load_textdomain('dual-chatbot', $fallback);
        }
    }
}, 1);

// As a safety net: enforce German labels/content on our settings page if translations didn’t load.
add_action('current_screen', function ($screen) {
    if (!isset($screen->id)) { return; }
    $is_settings_screen = (strpos((string)$screen->id, 'settings_page_dual-chatbot') !== false);
    $is_settings_page   = (!empty($_GET['page']) && strpos(sanitize_text_field((string) $_GET['page']), 'dual-chatbot') !== false);
    if (!$is_settings_screen && !$is_settings_page) { return; }
    $locale = function_exists('get_user_locale') ? get_user_locale() : get_locale();
    if (stripos((string)$locale, 'de') !== 0) { return; }
    // Only register fallback filters when explicitly enabled
    if ((int) get_option('dcb_force_de_labels', 0) !== 1) { return; }

    // Tab titles via context-aware gettext
    add_filter('gettext_with_context', function ($translation, $text, $context, $domain) {
        if ($domain !== 'dual-chatbot' || $context !== 'settings tab label') { return $translation; }
        $map = [
            'General'        => 'Allgemein',
            'Design'         => 'Design',
            'API'            => 'API',
    ...
    // Data & Privacy content strings
    add_filter('gettext', function ($translation, $text, $domain) {
        if ($domain !== 'dual-chatbot') { return $translation; }
        // Exact matches
        $map = [
            'Data & Privacy' => 'Daten & Privatsphäre',
            'Control analytics, data retention and user data requests (GDPR).' => 'Steuern Sie Analysen, Aufbewahrung und Auskunftsersuchen (DSGVO).',
            'Enable usage analytics (opt-in)' => 'Nutzungsanalysen aktivieren (Opt-in)',
            'Allow anonymized usage analytics (costs, tokens, latency).' => 'Anonymisierte Nutzungsanalysen erlauben (Kosten, Tokens, Latenz).',
            'Retention (days)' => 'Aufbewahrung (Tage)',
            'Older records will be purged automatically by a daily cleanup job.' => 'Ältere Einträge werden täglich automatisch bereinigt.',
            'User Data Requests (DSAR)' => 'Nutzerdaten-Anfragen (DSAR)',
            'User E-mail or User ID' => 'E-Mail des Nutzers oder Nutzer-ID',
            'Export User Data (JSON)' => 'Nutzerdaten exportieren (JSON)',
            'Delete User Data' => 'Nutzerdaten löschen',
            'Delete all saved records for this user? This cannot be undone.' => 'Alle gespeicherten Einträge für diesen Nutzer löschen? Dies kann nicht rückgängig gemacht werden.',
        ...
        // Robust partial match for the long paragraph
        if (strpos($text, 'We store chat metadata and messages for operation and quality') !== false) {
            return 'Für Betrieb und Qualität speichern wir Chat-Metadaten und Nachrichten. Analysen sind optional (Opt-in). Sie können die Daten eines Nutzers jederzeit exportieren oder löschen.';
        }
        return $translation;
    }, 10, 3);
});

// One-time i18n probe (QA): verify msgctxt translation for settings tab labels.
add_action('admin_init', function () {
    if (!current_user_can('manage_options')) { return; }
    if (get_option('dcb_i18n_probe_done') === '1') { return; }
    $probe = translate_with_gettext_context('Data & Privacy', 'settings tab label', 'dual-chatbot');
    error_log('[dual-chatbot] i18n probe (settings tab label): ' . $probe);
    update_option('dcb_i18n_probe_done', '1');
});

// Force German translations for our settings screen only (de_AT first, fallback de_DE). (legacy fallback)
if (false) {
add_action('current_screen', function ($screen) {
    // Prüfe, ob wir auf unserer Seite sind (passen, falls dein page-Slug abweicht):
    $is_settings_screen = (isset($screen->id) && strpos($screen->id, 'settings_page_dual-chatbot') !== false);
    $is_settings_page   = (!empty($_GET['page']) && strpos(sanitize_text_field((string) $_GET['page']), 'dual-chatbot') !== false);
    if (!$is_settings_screen && !$is_settings_page) return;
    ...
    // Kandidaten in Reihenfolge: exakte User-Locale, de_AT, de_DE
    $candidates = array_unique([
        $base . $domain . '-' . $loc . '.mo',
        $base . $domain . '-de_AT.mo',
        $base . $domain . '-de_DE.mo',
    ]);

    // Versuche, in die User-Locale zu wechseln (nur für diese Seite)
    ...
    // Enforce German tab/content labels on this screen if MO not compiled yet
    if (preg_match('/^de/i', (string) $loc)) {
        add_filter('gettext_with_context', function($translation, $text, $context, $d) {
            if ($d !== 'dual-chatbot' || $context !== 'settings tab label') { return $translation; }
            $map = [
                'General'        => 'Allgemein',
                'Design'         => 'Design',
                'API'            => 'API',
        ...
        add_filter('gettext', function($translation, $text, $d) {
            if ($d !== 'dual-chatbot') { return $translation; }
            $map = [
                'Data & Privacy' => 'Daten & Privatsphäre',
                'Control analytics, data retention and user data requests (GDPR).' => 'Steuern Sie Analysen, Aufbewahrung und Auskunftsersuchen (DSGVO).',
                'We store chat metadata and messages for operation and quality. Analytics is optional (opt-in). You can export or delete a user’s data at any time.' => 'Für Betrieb und Qualität speichern wir Chat-Metadaten und Nachrichten. Analysen sind optional (Opt-in). Sie können die Daten eines Nutzers jederzeit exportieren oder löschen.',
                'Enable usage analytics (opt-in)' => 'Nutzungsanalysen aktivieren (Opt-in)',
                'Allow anonymized usage analytics (costs, tokens, latency).' => 'Anonymisierte Nutzungsanalysen erlauben (Kosten, Tokens, Latenz).',
                'Retention (days)' => 'Aufbewahrung (Tage)',
                'Older records will be purged automatically by a daily cleanup job.' => 'Ältere Einträge werden täglich automatisch bereinigt.',
...
class Dual_Chatbot_Plugin {
    ...
    const OPTION_MAX_MESSAGE_LENGTH    = 'dual_chatbot_max_message_length';
    ...
    // site administrator to configure a thirdâ€‘party search service.  If left
    ...
    // Analytics
    const OPTION_ANALYTICS_OPTOUT       = 'dual_chatbot_analytics_optout';
    const OPTION_ANALYTICS_SALT         = 'dual_chatbot_analytics_salt';

    // Label displayed when advisor chat is minimized
    const OPTION_MINIMIZED_LABEL        = 'dual_chatbot_minimized_label';

    // Greeting options (admin > Allgemein > Begrüßung)
    const OPTION_GREETING_FAQ      = 'dual_chatbot_greeting_faq';
    const OPTION_GREETING_MEMBERS  = 'dual_chatbot_greeting_members';
    ...
    const DEFAULTS = [
        ...
        // UI text defaults
        // Greetings for initial assistant message (used when opening the chat)
        'dual_chatbot_greeting_faq'     => 'Hallo! Ich bin der FAQ-Bot. Wobei kann ich helfen?',
        'dual_chatbot_greeting_members' => 'Willkommen! Ich bin der Mitglieder-Bot. Wie darf ich unterstützen?',
        ...
        // Validation
        'dual_chatbot_max_message_length' => 2000,
    ];
    ...
    public function __construct() {
        ...
        // Load translations as early as possible after plugins are loaded
        add_action('plugins_loaded', [$this, 'load_textdomain']);
        add_action('init', [$this, 'maybe_schedule_cron']);
        add_action('init', [$this, 'maybe_upgrade_schema']);
        add_action('admin_menu', [$this, 'add_admin_menu']);
        add_action('admin_init', [$this, 'register_settings']);
        add_action('admin_init', [$this, 'enforce_no_autoload_for_keys']);
        ...
        // Ensure analytics table exists after updates
        add_action('init', [$this, 'maybe_install_analytics_table']);
        add_action('dual_chatbot_daily_maintenance', [$this, 'handle_daily_maintenance']);
    }
    ...
    public static function activate(): void {
        ...
        dbDelta($sql3);
        // Analytics table (privacy-friendly, no content)
        $analytics_table = $wpdb->prefix . 'dual_chatbot_analytics';
        $sql4 = "CREATE TABLE $analytics_table (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        ...
        dbDelta($sql4);
        // Ensure a private salt for client id hashing exists
        if (!get_option(self::OPTION_ANALYTICS_SALT)) {
            $salt = wp_generate_password(64, true, true);
            update_option(self::OPTION_ANALYTICS_SALT, $salt, false);
        }
        // Ensure rewrite rules reflect any custom pages/permalinks
        flush_rewrite_rules();
        // Schedule cron
        if (!wp_next_scheduled('dual_chatbot_daily_maintenance')) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', 'dual_chatbot_daily_maintenance');
        }
    }

    /** Ensure analytics table and salt exist even on upgrade. */
    public function maybe_install_analytics_table(): void {
        global $wpdb;
        $table = $wpdb->prefix . 'dual_chatbot_analytics';
        $exists = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(1) FROM information_schema.TABLES WHERE TABLE_SCHEMA=%s AND TABLE_NAME=%s",
            $wpdb->dbname ? $wpdb->dbname : DB_NAME,
            $table
        ));
        if ((int)$exists === 0) {
            require_once ABSPATH . 'wp-admin/includes/upgrade.php';
        ...
        // Ensure 'type' column is VARCHAR(32) (migrate from ENUM if needed)
        try {
            $col = $wpdb->get_row("SHOW COLUMNS FROM $table LIKE 'type'");
            if ($col && stripos((string)$col->Type, 'varchar') === false) {
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $wpdb->query("ALTER TABLE `$table` MODIFY COLUMN `type` VARCHAR(32) NOT NULL"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
            }
        } catch (\Throwable $e) { /* ignore */ }
        // Ensure client_id column is wide enough for prefixed SHA-256 (e.g., 'u:'+64 hex = 66)
        try {
            $col = $wpdb->get_row("SHOW COLUMNS FROM $table LIKE 'client_id'");
            if ($col) {
                $type = (string) $col->Type; // e.g., char(64) or varchar(64)
                if (!preg_match('/varchar\((\d+)\)/i', $type) || (int)preg_replace('/\D+/', '', $type) < 66) {
                    // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                    $wpdb->query("ALTER TABLE `$table` MODIFY COLUMN `client_id` VARCHAR(128) NOT NULL DEFAULT ''"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
                }
            }
        } catch (\Throwable $e) { /* ignore */ }
        if (!get_option(self::OPTION_ANALYTICS_SALT)) {
    ...
    /** Schedule maintenance if missing (idempotent). */
    ...
    /** Delete old analytics/history/sessions according to retention (default 180 days). */
    public function handle_daily_maintenance(): void {
        $days = (int) apply_filters('dual_chatbot_retention_days', 180);
        if ($days < 30) { $days = 30; }
        $threshold_ts = current_time('timestamp') - ($days * DAY_IN_SECONDS);
        $threshold = date('Y-m-d H:i:s', $threshold_ts);
        global $wpdb;
        $analytics = $wpdb->prefix . 'dual_chatbot_analytics';
        $history   = $wpdb->prefix . 'chatbot_history';
        $sessions  = $wpdb->prefix . 'chatbot_sessions';
        // Delete old analytics
        $wpdb->query($wpdb->prepare("DELETE FROM $analytics WHERE ts < %s", $threshold));
        // Delete old messages
        $wpdb->query($wpdb->prepare("DELETE FROM $history WHERE `timestamp` < %s", $threshold));
        // Delete old sessions (no recent activity)
        $wpdb->query($wpdb->prepare("DELETE FROM $sessions WHERE `updated_at` < %s", $threshold));
    }

    /** Check whether a specific index exists on a table. */
    private function index_exists(string $table, string $index_name): bool {
        global $wpdb;
        $sql = $wpdb->prepare("SHOW INDEX FROM `$table` WHERE Key_name = %s", $index_name);
        return (bool) $wpdb->get_var($sql);
    }

    /** Create an index if it does not already exist (idempotent). */
    private function ensure_index(string $table, string $index_name, string $columns_sql): void {
        if (!$this->index_exists($table, $index_name)) {
            global $wpdb;
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->query("CREATE INDEX $index_name ON `$table` ($columns_sql)"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
        }
    }

    /** Best-effort schema upgrades: collation and missing indexes/columns. */
    public function maybe_upgrade_schema(): void {
        global $wpdb;
        // Version gate for upgrades
        $installed = get_option('dual_chatbot_db_version', '0');
        $target    = '1.2.0';

        // Prefer utf8mb4_unicode_520_ci if available
        $collation = 'utf8mb4_unicode_520_ci';
        try {
            $ok = $wpdb->get_var("SHOW COLLATION LIKE 'utf8mb4_unicode_520_ci'");
            if (!$ok) { $collation = ''; }
        } catch (\Throwable $e) { $collation = ''; }
        $tables = [
            $wpdb->prefix . 'chatbot_history',
            $wpdb->prefix . 'chatbot_sessions',
            $wpdb->prefix . 'dual_chatbot_analytics',
            $wpdb->prefix . 'chatbot_knowledge',
        ];
        if ($collation) {
            foreach ($tables as $t) {
                try { $wpdb->query("ALTER TABLE `$t` CONVERT TO CHARACTER SET utf8mb4 COLLATE $collation"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
                } catch (\Throwable $e) { /* ignore */ }
            }
        }
        // Idempotent indexes
        $this->ensure_index($wpdb->prefix . 'chatbot_history',        'idx_history_userpid',   'user_pid');
        $this->ensure_index($wpdb->prefix . 'chatbot_history',        'idx_history_session',   'session_id');
        $this->ensure_index($wpdb->prefix . 'chatbot_sessions',       'idx_sessions_userpid',  'user_pid');
        $this->ensure_index($wpdb->prefix . 'dual_chatbot_analytics', 'idx_analytics_ts',      'ts');
        $this->ensure_index($wpdb->prefix . 'dual_chatbot_analytics', 'idx_analytics_session', 'session_id');

        if (version_compare($installed, $target, '<')) {
            update_option('dual_chatbot_db_version', $target, true);
        }
    }
...
public function enqueue_frontend_assets(): void {
    ...
    // Prefer minified assets if they exist (built via esbuild), fallback to sources
    $use_min = !(defined('SCRIPT_DEBUG') && SCRIPT_DEBUG);
    $style_rel   = $use_min && file_exists(plugin_dir_path(__FILE__) . 'assets/css/chatbot.min.css') ? 'assets/css/chatbot.min.css' : 'assets/css/chatbot.css';
    $script_rel  = $use_min && file_exists(plugin_dir_path(__FILE__) . 'assets/js/chatbot.min.js') ? 'assets/js/chatbot.min.js' : 'assets/js/chatbot.js';
    ...
    // ALLE Daten in EINEM Aufruf bündeln
        ...
        wp_localize_script(
            ...
            'DualChatbotConfig',
            [
            'restUrl'           => esc_url_raw(rest_url(Dual_Chatbot_Rest_API::public_base())),
            'analyticsRestUrl'  => esc_url_raw(rest_url(Dual_Chatbot_Rest_API::internal_base())),
            'logRestUrl'        => esc_url_raw(rest_url(Dual_Chatbot_Rest_API::internal_base() . '/client-log')),
            'nonce'             => wp_create_nonce('wp_rest'),
            'analyticsEnabled'  => (get_option(self::OPTION_ANALYTICS_OPTOUT, '0') !== '1'),
            'debugEnabled'      => (get_option(self::OPTION_DEBUG_LOG, '0') === '1'),
            'maxMessageLength'  => (int) get_option(self::OPTION_MAX_MESSAGE_LENGTH, self::DEFAULTS[self::OPTION_MAX_MESSAGE_LENGTH] ?? 2000),
            'faqEnabled'        => ($faq_enabled === '1'),
            'advisorEnabled'    => ($advisor_enabled === '1'),
            'mode'              => ($advisor_enabled === '1' ? 'advisor' : (($faq_enabled === '1') ? 'faq' : 'none')),
            'whisperApiUrl'     => esc_url_raw(rest_url(Dual_Chatbot_Rest_API::public_base() . '/transcribe_audio')),
            ...
            // removed: autoOpenDelay â€“ option deprecated
            ...
            // Greetings passed to JS
            'greetingFaq'       => get_option(self::OPTION_GREETING_FAQ, self::DEFAULTS[self::OPTION_GREETING_FAQ] ?? ''),
            'greetingMembers'   => get_option(self::OPTION_GREETING_MEMBERS, self::DEFAULTS[self::OPTION_GREETING_MEMBERS] ?? ''),
            // A11y labels
            'labelSend'         => __('Senden', 'dual-chatbot'),
            'labelClose'        => __('Schließen', 'dual-chatbot'),
            'labelMinimize'     => __('Minimieren', 'dual-chatbot'),
            'labelMicrophone'   => __('Mikrofon', 'dual-chatbot'),
            'labelStop'         => __('Stopp', 'dual-chatbot'),
            'labelAccept'       => __('Übernehmen', 'dual-chatbot'),
            'labelCancel'       => __('Abbrechen', 'dual-chatbot'),
            'ariaChatLog'       => __('Chatverlauf', 'dual-chatbot'),
        ]
    );
}
 ...
 /**
    ...
    /** Sanitize boolean-like values to string '1' or '0'. */
    public static function sanitize_bool_str($value): string {
        $b = false;
        if (function_exists('rest_sanitize_boolean')) {
            $b = rest_sanitize_boolean($value);
        } else {
    ...
    /** Sanitize enum values by whitelist, otherwise return default. */
    public static function sanitize_enum($value, array $allowed, string $default): string {
        $v = is_string($value) ? trim($value) : '';
        return in_array($v, $allowed, true) ? $v : $default;
    }

    /**
     * Sanitize secret options (API keys). If the submitted value is empty or looks like a masked placeholder,
     * keep the existing stored value. Otherwise, trim and store the new value. Also enforce autoload = 'no'.
     */
    public static function sanitize_secret($value, string $option_name): string {
        $raw = is_string($value) ? trim($value) : '';
        $current = get_option($option_name, '');
        // Detect bullet-masked placeholder (e.g., ••••1234) or empty
        $is_masked = (bool) preg_match('/^•{3,}[A-Za-z0-9+\/=\-]{0,8}$/u', $raw);
        $final = ($raw === '' || $is_masked) ? (string) $current : $raw;
        // Ensure autoload is disabled for secrets
        try { self::set_option_no_autoload($option_name); } catch (\Throwable $e) { /* ignore */ }
        return $final;
    }

    /** Mask a secret for UI: returns bullets plus last 4 characters. */
    public static function mask_secret(string $secret): string {
        $s = trim((string) $secret);
        if ($s === '') return '';
        $last4 = substr($s, -4);
        return '••••' . $last4;
    }

    /** Enforce autoload = 'no' for secret options. */
    public function enforce_no_autoload_for_keys(): void {
        $keys = [
            self::OPTION_OPENAI_API_KEY,
            self::OPTION_WHISPER_API_KEY,
            self::OPTION_MYAPLEFY_API_KEY,
            self::OPTION_WEB_SEARCH_API_KEY,
        ];
        foreach ($keys as $opt) {
            try { self::set_option_no_autoload($opt); } catch (\Throwable $e) { /* ignore */ }
        }
    }

    private static function set_option_no_autoload(string $option_name): void {
        global $wpdb;
    ...
    public function enqueue_admin_assets(string $hook): void {
        ...
        // Reuse frontend stylesheet for the admin live preview (no JS)
        ...
        // Hide greeting live preview per request (remove or separate) -> remove for now
        wp_add_inline_style('dual-chatbot-style', '.dual-chatbot-admin-preview{display:none!important}');
    }
    ...
    public function register_settings(): void {
        // General section.
        register_setting('dual_chatbot_options', self::OPTION_ENABLED, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        register_setting('dual_chatbot_options', self::OPTION_FAQ_ENABLED, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        register_setting('dual_chatbot_options', self::OPTION_ADVISOR_ENABLED, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        ...
        // Secrets: keep autoload off and do not overwrite when empty/masked
        register_setting('dual_chatbot_options', self::OPTION_OPENAI_API_KEY, [
            'type' => 'string',
            'sanitize_callback' => function($v){ return Dual_Chatbot_Plugin::sanitize_secret($v, self::OPTION_OPENAI_API_KEY); }
        ]);
        register_setting('dual_chatbot_options', self::OPTION_WHISPER_API_KEY, [
            'type' => 'string',
            'sanitize_callback' => function($v){ return Dual_Chatbot_Plugin::sanitize_secret($v, self::OPTION_WHISPER_API_KEY); }
        ]);
        register_setting('dual_chatbot_options', self::OPTION_MYAPLEFY_API_KEY, [
            'type' => 'string',
            'sanitize_callback' => function($v){ return Dual_Chatbot_Plugin::sanitize_secret($v, self::OPTION_MYAPLEFY_API_KEY); }
        ]);
        ...
        // Validation constraints
        register_setting('dual_chatbot_options', self::OPTION_MAX_MESSAGE_LENGTH, [
            'type' => 'integer',
        ...
        // New greeting settings (separate for FAQ and Members/Advisor)
        register_setting('dual_chatbot_options', self::OPTION_GREETING_FAQ, ['type' => 'string', 'sanitize_callback' => 'wp_kses_post']);
        register_setting('dual_chatbot_options', self::OPTION_GREETING_MEMBERS, ['type' => 'string', 'sanitize_callback' => 'wp_kses_post']);
        ...
        register_setting('dual_chatbot_options', self::OPTION_PRECHAT_ENABLED, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        register_setting('dual_chatbot_options', self::OPTION_PRECHAT_REQUIRE_NAME, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        register_setting('dual_chatbot_options', self::OPTION_PRECHAT_REQUIRE_EMAIL, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);

        // Tools settings.
        register_setting('dual_chatbot_options', self::OPTION_SIMULATE_MEMBERSHIP, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        register_setting('dual_chatbot_options', self::OPTION_DEBUG_LOG, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);

        // Web search settings
        register_setting('dual_chatbot_options', self::OPTION_WEB_SEARCH_ENABLED, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        ...
        // specify an API key and endpoint for a thirdâ€‘party search service.
        // If both fields are empty the plugin falls back to the default
        // DuckDuckGo search without authentication.
        register_setting('dual_chatbot_options', self::OPTION_WEB_SEARCH_API_KEY, [
            'type' => 'string',
            'sanitize_callback' => function($v){ return Dual_Chatbot_Plugin::sanitize_secret($v, self::OPTION_WEB_SEARCH_API_KEY); }
        ]);
        ...
        register_setting('dual_chatbot_options', self::OPTION_ANALYTICS_OPTOUT, ['type' => 'string', 'sanitize_callback' => [self::class, 'sanitize_bool_str']]);
        ...
        register_setting('dual_chatbot_options', self::OPTION_FAQ_MODEL, ['type' => 'string', 'sanitize_callback' => function($v){
            return Dual_Chatbot_Plugin::sanitize_enum($v, ['gpt-3.5-turbo','gpt-4','gpt-4o'], 'gpt-3.5-turbo');
        }]);
        register_setting('dual_chatbot_options', self::OPTION_ADVISOR_MODEL, ['type' => 'string', 'sanitize_callback' => function($v){
            return Dual_Chatbot_Plugin::sanitize_enum($v, ['gpt-3.5-turbo','gpt-4','gpt-4o'], 'gpt-4o');
        }]);
        ...
        // Max message length
        add_settings_field(self::OPTION_MAX_MESSAGE_LENGTH, __('Maximale Eingabelänge', 'dual-chatbot'), function() {
            $value = (int) get_option(self::OPTION_MAX_MESSAGE_LENGTH, self::DEFAULTS[self::OPTION_MAX_MESSAGE_LENGTH] ?? 2000);
            $value = $value > 0 ? $value : 2000;
            echo '<input type="number" class="small-text" name="' . esc_attr(self::OPTION_MAX_MESSAGE_LENGTH) . '" value="' . esc_attr($value) . '" min="50" max="8000" step="50" />';
            echo '<p class="description">' . esc_html__('Maximale Zeichenanzahl je Nutzer-Nachricht (50–8000).', 'dual-chatbot') . '</p>';
        }, 'dual-chatbot-settings-general', 'dual_chatbot_general');

        // Begrüßung section inside Allgemein tab
        add_settings_section('dual_chatbot_greeting', __('Begrüßung', 'dual-chatbot'), function() {
            echo '<p>' . esc_html__('Konfiguriere die anfänglichen Begrüßungstexte für FAQ- und Mitglieder-Bot. Die Texte werden beim Öffnen des Chats angezeigt.', 'dual-chatbot') . '</p>';
        }, 'dual-chatbot-settings-general');

        add_settings_field(self::OPTION_GREETING_FAQ, __('Begrüßung (FAQ-Bot)', 'dual-chatbot'), function() {
            $default = __('Hallo! Ich bin der FAQ‘Bot. Wobei kann ich helfen?', 'dual-chatbot');
            $value = get_option(self::OPTION_GREETING_FAQ, $default);
            echo '<div class="wp-editor-container">';
            echo '<textarea class="large-text" rows="4" name="' . esc_attr(self::OPTION_GREETING_FAQ) . '">' . esc_textarea($value) . '</textarea>';
            echo '</div>';
            echo '<p class="description">' . esc_html__('Mehrzeiliger Text, der beim öffnen des FAQ-Chats erscheint.', 'dual-chatbot') . '</p>';
        }, 'dual-chatbot-settings-general', 'dual_chatbot_greeting');

        add_settings_field(self::OPTION_GREETING_MEMBERS, __('Begrüßung (Mitglieder-Bot)', 'dual-chatbot'), function() {
            $default = __('Willkommen! Ich bin der Mitglieder‘Bot. Wie darf ich unterstützen?', 'dual-chatbot');
            $value = get_option(self::OPTION_GREETING_MEMBERS, $default);
            echo '<div class="wp-editor-container">';
            echo '<textarea class="large-text" rows="4" name="' . esc_attr(self::OPTION_GREETING_MEMBERS) . '">' . esc_textarea($value) . '</textarea>';
            echo '</div>';
            echo '<p class="description">' . esc_html__('Mehrzeiliger Text, der beim Öffnen des Mitglieder-Chats erscheint.', 'dual-chatbot') . '</p>';
            // Live preview (server-rendered, no inline JS)
            $faq_greeting = get_option(self::OPTION_GREETING_FAQ, __('Hallo! Ich bin der FAQ‘Bot. Wobei kann ich helfen?', 'dual-chatbot'));
            $mem_greeting = get_option(self::OPTION_GREETING_MEMBERS, __('Willkommen! Ich bin der Mitglieder‘Bot. Wie darf ich unterstützen?', 'dual-chatbot'));
            echo '<div class="dual-chatbot-admin-preview" style="margin-top:12px; padding:12px; background:#fff; border:1px solid #e5e7eb; border-radius:8px;">';
            echo '<div style="font-weight:600; margin-bottom:8px;">' . esc_html__('Live-Vorschau', 'dual-chatbot') . '</div>';
            echo '<div class="dual-chatbot-main-chat" style="max-height:220px; overflow:auto; background:#fafafa; padding:14px; border-radius:8px;">';
            echo '  <div class="dual-chatbot-message dual-chatbot-bot"><div class="dual-chatbot-message-content">' . wp_kses_post($faq_greeting) . '</div></div>';
            echo '  <div class="dual-chatbot-message dual-chatbot-bot"><div class="dual-chatbot-message-content">' . wp_kses_post($mem_greeting) . '</div></div>';
            echo '</div>';
            echo '</div>';
        }, 'dual-chatbot-settings-general', 'dual_chatbot_greeting');
        ...
        */
        ...
        add_settings_field(self::OPTION_DEBUG_LOG, __('Debug-Log aktivieren', 'dual-chatbot'), function() {
            ...
            echo '<span class="description">' . esc_html__('Protokolliere API-Aufrufe und Fehler in eine Log-Datei. Pfad: wp-content/uploads/chatbot-debug.log. Erfasst zusätzlich Browser-Fehler (Client-Logs).', 'dual-chatbot') . '</span>';
        }, 'dual-chatbot-settings-tools', 'dual_chatbot_tools');
        ...
        add_settings_field(self::OPTION_WEB_SEARCH_RESULTS, __('Anzahl Suchergebnisse', 'dual-chatbot'), function() {
            ...
            echo '<p class="description">' . esc_html__('Wie viele Suchergebnisse sollen maximal als Quellen erscheinen? (“5”)', 'dual-chatbot') . '</p>';
        }, 'dual-chatbot-settings-behavior', 'dual_chatbot_behavior');
        ...
        add_settings_section('dual_chatbot_appearance', __('Popup & Erscheinungsbild', 'dual-chatbot'), function() {
            ...
            echo '<p class="description">' . esc_html__('Hinweis: Leere Farbkästchen nutzen automatisch die im Stylesheet definierten Standardfarben. „Auf Standard zurücksetzen“ löscht alle Overrides.', 'dual-chatbot') . '</p>';
    ...
    public function render_settings_page(): void {
        ...
        // Slugs only (no cached labels)
        $tabs = ['general','design','api','knowledge','analytics','privacy','tools','status'];
        // Lazy-translate labels at render time (ensures post-locale-switch translations)
        $tab_labels = [
            'general'   => __('General', 'dual-chatbot'),
            'design'    => __('Design', 'dual-chatbot'),
            'api'       => __('API', 'dual-chatbot'),
            'knowledge' => __('Knowledge Base', 'dual-chatbot'),
            'analytics' => __('Analytics', 'dual-chatbot'),
            'privacy'   => __('Data & Privacy', 'dual-chatbot'),
            'tools'     => __('Tools', 'dual-chatbot'),
        ];
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__('Chatbot Einstellungen', 'dual-chatbot') . '</h1>';
        // Show Settings API messages (errors/updated)
        settings_errors();
        if (isset($_GET['settings-updated']) && $_GET['settings-updated']) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Einstellungen gespeichert.', 'dual-chatbot') . '</p></div>';
        }
        // Tab navigation
        echo '<h2 class="nav-tab-wrapper">';
        $url_base = admin_url('options-general.php?page=dual-chatbot-settings');
        foreach ($tabs as $slug) {
            switch ($slug) {
                case 'general':   $label = esc_html__('Allgemein', 'dual-chatbot'); break;
                case 'design':    $label = esc_html__('Design', 'dual-chatbot'); break;
                case 'api':       $label = esc_html__('API', 'dual-chatbot'); break;
                case 'knowledge': $label = esc_html__('Wissensdatenbank', 'dual-chatbot'); break;
                case 'analytics': $label = esc_html__('Analysen', 'dual-chatbot'); break;
                case 'privacy':   $label = esc_html__('Daten & Privatsphäre', 'dual-chatbot'); break;
                case 'status':    $label = esc_html__('Status', 'dual-chatbot'); break;
                default:          $label = esc_html__('Werkzeuge', 'dual-chatbot'); break;
            }
            $class = ($current_tab === $slug) ? ' nav-tab-active' : '';
            echo '<a class="nav-tab' . esc_attr($class) . '" href="' . esc_url($url_base . '&tab=' . $slug) . '">' . $label . '</a>';
        }
        echo '</h2>';
        // Per-tab content; tabs with settings have their own form posting to options.php
        foreach ($tabs as $slug) {
            $style = ($current_tab === $slug) ? '' : 'style="display:none;"';
            echo '<div class="dual-chatbot-tab-content" id="dual-chatbot-tab-' . esc_attr($slug) . '" ' . $style . '>'; 
            if ($slug === 'status') {
                if (!function_exists('dual_chatbot_render_status_section')) {
                    require_once __DIR__ . '/includes/admin/status-page.php';
                }
                if (function_exists('dual_chatbot_render_status_section')) {
                    dual_chatbot_render_status_section();
                }
            } else {
                echo '<form method="post" action="options.php" enctype="multipart/form-data">';
                settings_fields('dual_chatbot_options');
                // Referer to return to the current tab after saving
                $ref = admin_url('options-general.php?page=dual-chatbot-settings&tab=' . $slug);
                echo '<input type="hidden" name="_wp_http_referer" value="' . esc_url($ref) . '" />';
                if ($slug === 'analytics') {
                    $this->render_analytics_tab();
                } elseif ($slug === 'api') {
                    // Prevent leaking current key values into value="..." attributes
                    add_filter('option_' . self::OPTION_WEB_SEARCH_API_KEY, '__return_empty_string');
                    do_settings_sections('dual-chatbot-settings-api');
                    // Set masked placeholder for Websuche API-Key via JS
                    $ws_stored = get_option(self::OPTION_WEB_SEARCH_API_KEY, '');
                    $ws_masked = esc_js(self::mask_secret((string)$ws_stored));
                    echo '<script>(function(){try{var name=' . wp_json_encode(self::OPTION_WEB_SEARCH_API_KEY) . ';var el=document.querySelector("input[name=' . "'" . '"+name+"' . "'" . ']"); if(el && !el.value){ el.placeholder = ' . wp_json_encode($ws_masked) . '; }}catch(e){}})();</script>';
                } else {
                    do_settings_sections('dual-chatbot-settings-' . $slug);
                }
                submit_button();
                echo '</form>';
            }
    ...
    /**
     * Render the Analytics tab UI with period filter and KPI cards.
     */
    private function render_analytics_tab(): void {
        ...
        echo '<div class="dual-chatbot-analytics">';
        echo '<h2 class="title">' . esc_html__('Analytics', 'dual-chatbot') . '</h2>';
        echo '<p>' . esc_html__('Datenschutzfreundliche Nutzungsmetriken. Inhalte werden nicht gespeichert.', 'dual-chatbot') . '</p>';

        // Opt-out checkbox
        $optout = get_option(self::OPTION_ANALYTICS_OPTOUT, '0') === '1';
        echo '<input type="hidden" name="' . esc_attr(self::OPTION_ANALYTICS_OPTOUT) . '" value="0" />';
        echo '<label style="display:block;margin:12px 0;">';
        echo '<input type="checkbox" name="' . esc_attr(self::OPTION_ANALYTICS_OPTOUT) . '" value="1" ' . checked($optout, true, false) . ' /> ';
        echo esc_html__('Tracking deaktivieren (Opt‑out)', 'dual-chatbot');
        echo '</label>';
        echo '<p class="description">' . esc_html__('Wenn aktiviert, sendet der Client keine Analytics‑Events.', 'dual-chatbot') . '</p>';

        // Filter select
        $base = admin_url('options-general.php?page=dual-chatbot-settings&tab=analytics');
        echo '<div style="margin:16px 0;">';
        echo '<label for="dual-analytics-range" style="margin-right:8px;">' . esc_html__('Zeitraum', 'dual-chatbot') . ':</label>';
        echo '<select id="dual-analytics-range" onchange="location.href=\'' . esc_url($base) . '&range=\'+this.value;">';
        $opts = [ 'today' => __('Heute', 'dual-chatbot'), '7d' => __('7 Tage', 'dual-chatbot'), '30d' => __('30 Tage', 'dual-chatbot') ];
        foreach ($opts as $k => $lbl) {
            echo '<option value="' . esc_attr($k) . '" ' . selected($range, $k, false) . '>' . esc_html($lbl) . '</option>';
        }
        echo '</select>';
        echo '</div>';

        $metrics = $this->compute_analytics_metrics($range);

        // KPI cards
        $card_css = 'display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;';
        echo '<div class="dual-analytics-cards" style="' . esc_attr($card_css) . '">';
        $cards = [
            [ __('Sessions pro User (Ø)', 'dual-chatbot'), isset($metrics['sessions_per_user_avg']) ? number_format_i18n($metrics['sessions_per_user_avg'], 2) : '0.00', __('Summe Sessions / Distinct Nutzer', 'dual-chatbot') ],
            [ __('Nachrichten pro Session (Ø)', 'dual-chatbot'), isset($metrics['messages_per_session_avg']) ? number_format_i18n($metrics['messages_per_session_avg'], 2) : '0.00', __('User+Bot Nachrichten / Session', 'dual-chatbot') ],
            [ __('Durchschnittliche Antwortzeit', 'dual-chatbot'), isset($metrics['avg_latency_ms']) ? number_format_i18n(round($metrics['avg_latency_ms'] / 1000, 2), 2) . ' s' : __('n/v', 'dual-chatbot'), __('Zeit bis zur ersten Bot‑Antwort', 'dual-chatbot') ],
            [ __('KB‑Anteil (Hit‑Rate)', 'dual-chatbot'), isset($metrics['kb_hit_rate']) ? number_format_i18n(round($metrics['kb_hit_rate'] * 100, 1), 1) . '%' : '0%', __('Anteil Bot‑Antworten mit Knowledge‑Base', 'dual-chatbot') ],
        ];
        foreach ($cards as $c) {
            echo '<div class="card" style="background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:14px;">';
            echo '<div style="font-size:13px;color:#555;margin-bottom:6px;">' . esc_html($c[0]) . '</div>';
            echo '<div style="font-size:24px;font-weight:600;">' . esc_html($c[1]) . '</div>';
            echo '<div style="font-size:12px;color:#777;margin-top:4px;">' . esc_html($c[2]) . '</div>';
            echo '</div>';
        }
        echo '</div>';
        echo '</div>';
    ...
    /**
     * Compute analytics metrics for a given period.
     * @param string $range 'today'|'7d'|'30d'
     * @return array<string,mixed>
     */
    private function compute_analytics_metrics(string $range): array {
        global $wpdb;
        $table = $wpdb->prefix . 'dual_chatbot_analytics';
        $now = current_time('mysql');
        // Determine start datetime
        if ($range === '7d') {
            $start = gmdate('Y-m-d H:i:s', time() - 7*DAY_IN_SECONDS + ( get_option('gmt_offset') * HOUR_IN_SECONDS ));
        } elseif ($range === '30d') {
            $start = gmdate('Y-m-d H:i:s', time() - 30*DAY_IN_SECONDS + ( get_option('gmt_offset') * HOUR_IN_SECONDS ));
        } else {
        ...
        // Always use prepare() for dynamic values (defense-in-depth)
        // Sessions per user average: (#distinct sessions) / (#distinct users)
        $distinct_sessions = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(DISTINCT session_id) FROM $table WHERE ts BETWEEN %s AND %s", $start, $now)
        );
        $distinct_users = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(DISTINCT COALESCE(NULLIF(user_id,0), CONCAT('c:', client_id))) FROM $table WHERE ts BETWEEN %s AND %s", $start, $now)
        );
        $sessions_per_user_avg = ($distinct_users > 0) ? ($distinct_sessions / $distinct_users) : 0.0;

        // Messages per session average: count of message_* divided by distinct sessions with any messages
        $msg_counts = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM $table WHERE ts BETWEEN %s AND %s AND type IN ('message_user','message_bot')", $start, $now)
        );
        $msg_sessions = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(DISTINCT session_id) FROM $table WHERE ts BETWEEN %s AND %s AND type IN ('message_user','message_bot')", $start, $now)
        );
        $messages_per_session_avg = ($msg_sessions > 0) ? ($msg_counts / $msg_sessions) : 0.0;

        // Average latency from bot messages with latency
        $avg_latency_ms = (float) $wpdb->get_var(
            $wpdb->prepare("SELECT AVG(latency_ms) FROM $table WHERE ts BETWEEN %s AND %s AND type='message_bot' AND latency_ms IS NOT NULL", $start, $now)
        );

        // KB hit rate: percentage of bot messages with kb_hit=1
        $bot_total = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM $table WHERE ts BETWEEN %s AND %s AND type='message_bot'", $start, $now)
        );
        $bot_kb = (int) $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM $table WHERE ts BETWEEN %s AND %s AND type='message_bot' AND kb_hit=1", $start, $now)
        );
        $kb_hit_rate = ($bot_total > 0) ? ($bot_kb / $bot_total) : 0.0;

        return [
            'sessions_per_user_avg'   => (float) $sessions_per_user_avg,
            'messages_per_session_avg'=> (float) $messages_per_session_avg,
            'avg_latency_ms'          => (float) $avg_latency_ms,
            'kb_hit_rate'             => (float) $kb_hit_rate,
        ];
    }
    ...
    public function render_file_upload_field(): void {
        // Nonce to protect file upload handling
        wp_nonce_field('dual_chatbot_indexer', '_dcb_idx_nonce');
        echo '<input type="file" name="dual_chatbot_documents[]" multiple="multiple" accept=".txt,.pdf,.md,.docx,.csv" />';
...
// Admin notice for missing PDF extraction tools (triggered by knowledge indexer)
add_action('admin_init', function () {
    if (!current_user_can('manage_options')) { return; }
    if (isset($_GET['dcb_pdf_notice']) && $_GET['dcb_pdf_notice'] === 'dismiss') {
        if (check_admin_referer('dcb_pdf_notice')) {
            delete_transient('dcb_pdf_extract_missing');
            delete_option('dcb_pdf_extract_missing_time');
...
add_action('admin_notices', function () {
    if (!current_user_can('manage_options')) { return; }
    if (!get_transient('dcb_pdf_extract_missing')) { return; }
    $dismiss_url = wp_nonce_url(add_query_arg(['dcb_pdf_notice' => 'dismiss']), 'dcb_pdf_notice');
    echo '<div class="notice notice-warning">'
        . '<p>' . esc_html__('PDF-Extraktion nicht verfuegbar. Bitte installieren Sie entweder die PHP-Bibliothek Smalot\\PdfParser oder das Server-Tool "pdftotext".', 'dual-chatbot') . '</p>'
        . '<p><a class="button button-secondary" href="' . esc_url($dismiss_url) . '">' . esc_html__('Verstanden', 'dual-chatbot') . '</a></p>'
        . '</div>';
});
...
function dual_chatbot_handle_cleanup(): void {
    ...
    global $wpdb;
    $history_table = $wpdb->prefix . 'chatbot_history';
    $safe = (int) get_option('dcb_safe_cleanup', 0);
    if ($safe) {
        $total = 0;
        $start = microtime(true);
        while (microtime(true) - $start < 5) {
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - Controlled table name, no user input
            $affected = $wpdb->query("DELETE FROM `$history_table` ORDER BY id ASC LIMIT 500");
            if ($affected === false) {
                wp_send_json_error(['message' => __('Datenbankfehler beim Löschen.', 'dual-chatbot')], 500);
            }
            $total += (int) $affected;
            if ((int) $affected === 0) { break; }
        }
        wp_send_json_success(['message' => sprintf(__('Löschung abgeschlossen. %d Einträge gelöscht.', 'dual-chatbot'), $total)]);
    } else {
        $deleted = $wpdb->query("TRUNCATE TABLE $history_table"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
        if ($deleted === false) {
            wp_send_json_error(['message' => __('Datenbankfehler beim Löschen.', 'dual-chatbot')], 500);
        } else {
...
function dual_chatbot_log(string $message): void {
    ...
    // Destination can be overridden for multi-server setups
    $dest = apply_filters('dual_chatbot_log_destination', 'file'); // 'file' | 'error_log'
    if ($dest === 'error_log') {
        @error_log(trim($line));
        return;
    }
    $path = apply_filters('dual_chatbot_log_path', '');
    if (!is_string($path) || $path === '') {
        $upload = wp_upload_dir();
        $path = trailingslashit($upload['basedir']) . 'chatbot-debug.log';
    }
    // Ensure directory exists
    $dir = dirname($path);
    if (!is_dir($dir)) { @wp_mkdir_p($dir); }
    $ok = @file_put_contents($path, $line, FILE_APPEND | LOCK_EX);
    if ($ok === false) { @error_log(trim($line)); }
}
...
function dual_chatbot_handle_clear_cache(): void {
    ...
    if ($safe) {
        ...
        while (microtime(true) - $start < 5) {
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - Controlled table name, no user input
            $affected = $wpdb->query("DELETE FROM `$knowledge_table` ORDER BY id ASC LIMIT 500");
            if ($affected === false) {
                wp_send_json_error(['message' => __('Datenbankfehler beim Löschen.', 'dual-chatbot')], 500);
            }
            $total += (int) $affected;
        ...
        wp_send_json_success(['message' => sprintf(__('Die Wissensdatenbank wurde geleert. %d Einträge gelöscht.', 'dual-chatbot'), $total)]);
    } else {
        $wpdb->query("TRUNCATE TABLE $knowledge_table"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared - DDL; no user input
        // Reset uploaded file list.
        update_option('dual_chatbot_uploaded_files', []);
...
function dual_chatbot_handle_reset_settings(): void {
    ...
    // Non-token color options â†’ remove
    $extra_color_opts = [
    ...
    $other_opts = [
        ...
        // Greetings
        Dual_Chatbot_Plugin::OPTION_GREETING_FAQ,
        Dual_Chatbot_Plugin::OPTION_GREETING_MEMBERS,
    ];
...
// Load Data & Privacy admin section and cron/DB helpers
require_once __DIR__ . '/includes/db.php';
require_once __DIR__ . '/includes/cron.php';
require_once __DIR__ . '/includes/admin/privacy-settings.php';
