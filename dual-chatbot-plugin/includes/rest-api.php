...
// === Helpers ================================================================
if (!function_exists('dcb_require_nonce')) {
    /**
     * Verify REST request via WP Nonce and optional capability.
     * Returns true on success or WP_Error on failure.
     */
    function dcb_require_nonce(WP_REST_Request $request, $capability = null) {
        $nonce = $request->get_header('x_wp_nonce');
        if (!$nonce || !wp_verify_nonce($nonce, 'wp_rest')) {
            return new WP_Error('dcb_rest_forbidden', __('Invalid or missing REST nonce.', 'dual-chatbot'), ['status' => 403]);
        }
        if ($capability && !current_user_can($capability)) {
            return new WP_Error('dcb_rest_cap', __('Insufficient permissions.', 'dual-chatbot'), ['status' => 403]);
        }
        return true;
    }
}

if (!function_exists('dcb_rate_limit_ok')) {
    /**
     * Lightweight per-minute rate limit bucket.
     */
    function dcb_rate_limit_ok(string $key, int $maxPerMinute = 30): bool {
        $bucket = 'dcb_rl_' . $key . '_' . gmdate('YmdHi');
        $count = (int) get_transient($bucket);
        if ($count >= $maxPerMinute) {
            return false;
        }
        set_transient($bucket, $count + 1, MINUTE_IN_SECONDS + 5);
        return true;
    }
}
...
class Dual_Chatbot_Rest_API {
    ...
    // Simple rate limit config for ch/v1 submit
    private const RL_MAX_REQUESTS = 20;            // max requests per window
    private const RL_WINDOW_SEC   = 5 * 60;        // 5 minutes
    ...
    public function register_routes(): void {
        ...
        register_rest_route($pub, '/check_membership', [
            ...
            'permission_callback' => function($request) {
                return dcb_require_nonce($request, 'read');
            },
        ]);
        ...
        // Lightweight compatibility routes under `ch/v1`
        register_rest_route('ch/v1', '/submit', [
            'methods'  => 'POST',
            'callback' => [$this, 'handle_ch_submit'],
            'permission_callback' => [$this, 'permission_submit'],
            'args'     => [
                'message'    => [ 'type' => 'string', 'required' => true ],
                'session_id' => [ 'type' => 'string', 'required' => false ],
                'context'    => [ 'type' => 'string', 'required' => false ],
        ...
        register_rest_route('ch/v1', '/sessions', [
            'methods'  => 'GET',
            'callback' => [$this, 'handle_ch_sessions'],
            'permission_callback' => [$this, 'permission_read'],
        ]);

        $allowed_contexts = ['faq','advisor','members','kb'];
        register_rest_route('ch/v1', '/context/(?P<context>[a-zA-Z0-9_-]+)', [
            'methods'  => 'GET',
            'callback' => [$this, 'handle_ch_context'],
            'permission_callback' => function($request) use ($allowed_contexts) {
                $context = sanitize_key((string) $request->get_param('context'));
                if (!in_array($context, $allowed_contexts, true)) {
                    return new WP_Error('dcb_invalid_context', __('Invalid context.', 'dual-chatbot'), ['status' => 400]);
                }
                // Simple rate-limit per IP for public route
                $ip = isset($_SERVER['REMOTE_ADDR']) ? sanitize_text_field((string) $_SERVER['REMOTE_ADDR']) : '0.0.0.0';
                $client_key = 'ip_' . $ip;
                if (function_exists('dcb_rate_limit_ok') && !dcb_rate_limit_ok('context_' . $client_key, 30)) {
                    return new WP_Error('dcb_rl', __('Too many requests, slow down.', 'dual-chatbot'), ['status' => 429]);
                }
                // Publicly allowed contexts only return generic, non-personal data.
                // If a context becomes private later, switch to: return dcb_require_nonce($request, 'read');
                return true;
            },
        ]);
    }
    ...
    public function handle_analytics_track(WP_REST_Request $request) {
        ...
        // Respect opt-in from Data & Privacy and legacy opt-out (both must allow)
        $opt_in = (int) get_option('dcb_analytics_opt_in', 0) === 1;
        $legacy_enabled = (get_option(Dual_Chatbot_Plugin::OPTION_ANALYTICS_OPTOUT, '0') !== '1');
        $enabled = $opt_in && $legacy_enabled;
        $enabled = (bool) apply_filters('dual_chatbot_analytics_enabled', $enabled);
        if (!$enabled) {
    ...
    private function analytics_insert_latency(string $type, string $session_id, int $latency_ms): void {
        // Respect opt-in + legacy opt-out via option + filter
        $opt_in = (int) get_option('dcb_analytics_opt_in', 0) === 1;
        $legacy_enabled = (get_option(Dual_Chatbot_Plugin::OPTION_ANALYTICS_OPTOUT, '0') !== '1');
        $enabled = $opt_in && $legacy_enabled;
        $enabled = (bool) apply_filters('dual_chatbot_analytics_enabled', $enabled);
    ...
    private function get_max_message_length(): int {
        $raw = (int) get_option(Dual_Chatbot_Plugin::OPTION_MAX_MESSAGE_LENGTH, 2000);
        $max = (int) apply_filters('dual_chatbot_max_message_len', $raw);
        if ($max < 50) { $max = 50; }
        if ($max > 8000) { $max = 8000; }
        return $max;
    }

    // Additional helpers for retry hints and daily cap
    private function rl_retry_after(string $scope): int {
        $win = (int) apply_filters('dual_chatbot_rl_window', self::RL_WINDOW_SEC);
        $key = 'ch_rl_' . md5($scope);
        $entry = get_transient($key);
        if (!is_array($entry)) { return max(1, $win); }
        $elapsed = time() - (int)($entry['start'] ?? time());
        return max(1, $win - $elapsed);
    }
    private function rl_daily_key(string $scope): string { return 'ch_rl_day_' . md5($scope) . '_' . gmdate('Ymd'); }
    private function rate_limit_check_daily_or_bump(string $scope): array {
        $max = (int) apply_filters('dual_chatbot_rl_daily_max', 500);
        if ($max <= 0) return [true, 0];
        $key = $this->rl_daily_key($scope);
        $count = (int) get_transient($key);
        if ($count >= $max) {
            $ttl = (int) (strtotime('tomorrow 00:00 UTC') - time());
    ...
    private function ensure_history_columns(): void {
        ...
        $this->ensure_index_silent($history_table, 'idx_history_userpid', 'user_pid');
        $this->ensure_index_silent($history_table, 'idx_client_msg', 'session_id, client_msg_id');
        $this->ensure_index_silent($history_table, 'idx_reply_to_client_msg', 'session_id, reply_to_client_msg_id');
        // session user_pid index if column was just added
        if ($need_userpid_sess) { $this->ensure_index_silent($sessions_table, 'idx_userpid', 'user_pid'); }
        $wpdb->suppress_errors($prev);
    }

    private function index_exists_local(string $table, string $index_name): bool {
        try {
            global $wpdb;
    ...
    private function ensure_index_silent(string $table, string $index_name, string $columns_sql): void {
        if (!$this->index_exists_local($table, $index_name)) {
            try { global $wpdb; // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                @ $wpdb->query("CREATE INDEX $index_name ON `$table` ($columns_sql)");
            } catch (\Throwable $e) { /* ignore */ }
        }
    }
     ...
     */
    public function permission_members_only(WP_REST_Request $request): bool {
        $nonce = $request->get_header('x_wp_nonce');
     ...
     */
    public function permission_advisor_or_public(WP_REST_Request $request): bool {
    ...
    public function handle_stream_message(WP_REST_Request $request) {
        ...
        if ($message === '') {
            return new WP_Error('ch_invalid', __('Leere Nachricht ist nicht erlaubt.', 'dual-chatbot'), ['status' => 400]);
        }
        if (function_exists('mb_strlen')) {
            if (mb_strlen($message, 'UTF-8') > $maxLen) {
                return new WP_Error('ch_invalid', sprintf(__('Nachricht ist zu lang (max. %d Zeichen).', 'dual-chatbot'), $maxLen), ['status' => 400]);
            }
        } else {
        ...
        try {
            ...
            if ($context === 'faq') {
                ...
                $allowed_models = (array) apply_filters('dual_chatbot_allowed_models', []);
                if (!empty($allowed_models) && !in_array($model, $allowed_models, true)) {
                    $first = reset($allowed_models);
                    if (is_string($first) && $first !== '') { $model = $first; }
                }
    ...
    public function handle_client_log(WP_REST_Request $request) {
        // Only process when debug logging is enabled (option + filter)
        $enabled = (get_option(Dual_Chatbot_Plugin::OPTION_DEBUG_LOG, '0') === '1');
        $enabled = (bool) apply_filters('dual_chatbot_analytics_enabled', $enabled);
        if (!$enabled) { return new WP_REST_Response(['ok' => true, 'skipped' => true], 200); }
    ...
    private function can_write(): bool {
        ...
        if (!$u || 0 === (int)$u->ID) return false;
        $roles = (array) $u->roles;
        return in_array('um_solo', $roles, true) || in_array('um_teams', $roles, true) || in_array('administrator', $roles, true);
    }
    private function can_read(): bool {
        return is_user_logged_in();
    }
    /** Fixed-window rate limiter via transients (filterable) */
    private function rate_limit_check_or_bump(string $scope): bool {
        $max = (int) apply_filters('dual_chatbot_rl_max', self::RL_MAX_REQUESTS);
        $win = (int) apply_filters('dual_chatbot_rl_window', self::RL_WINDOW_SEC);
        $key = 'ch_rl_' . md5($scope);
        $entry = get_transient($key);
        if (!is_array($entry)) {
            $entry = ['count' => 1, 'start' => time()];
            set_transient($key, $entry, $win);
            return true;
        }
        $elapsed = time() - (int)$entry['start'];
        if ($elapsed >= $win) {
            $entry = ['count' => 1, 'start' => time()];
            set_transient($key, $entry, $win);
            return true;
        ...
        $ttl = max(1, $win - $elapsed);
        set_transient($key, $entry, $ttl);
        return true;
    }

    // Permissions for ch/v1
    public function permission_read(WP_REST_Request $request): bool {
    ...
    public function permission_submit(WP_REST_Request $request) {
        if (!$this->can_write()) {
            return new WP_Error(
                'ch_forbidden',
                __('Deine Rolle erlaubt das Senden von Nachrichten nicht. (Erforderlich: SOLO/TEAMS/Administrator)', 'dual-chatbot'),
                ['status' => 403]
            );
        }
        $uid = get_current_user_id();
        $scope = $uid ? ('user:' . (string)$uid) : ('ip:' . $this->get_client_ip());
        if (!$this->rate_limit_check_or_bump($scope)) {
            $max = (int) apply_filters('dual_chatbot_rl_max', self::RL_MAX_REQUESTS);
            $win = (int) apply_filters('dual_chatbot_rl_window', self::RL_WINDOW_SEC);
            return new WP_Error(
                'ch_rate_limited',
                sprintf(__('Zu viele Anfragen. Erlaubt: %1$d in %2$d Minuten.', 'dual-chatbot'), $max, (int)($win/60)),
                ['status' => 429, 'retry_after' => $this->rl_retry_after($scope)]
            );
        }
        // Daily cap
        list($okDay, $ttlDay) = $this->rate_limit_check_daily_or_bump($scope);
        if (!$okDay) {
            return new WP_Error(
                'ch_daily_capped',
                sprintf(__('Tageslimit erreicht. Versuche es in %d Minuten erneut.', 'dual-chatbot'), (int)ceil($ttlDay/60)),
                ['status' => 429, 'retry_after' => $ttlDay]
            );
        }
        return true;
    }

    // === Compatibility handlers for ch/v1 ===
    public function handle_ch_submit(WP_REST_Request $request) {
        try {
            // Default to advisor context
            $ctx = sanitize_text_field((string) ($request->get_param('context') ?: 'advisor'));
            if (method_exists($request, 'set_param')) { $request->set_param('context', $ctx); }
            $resp = $this->handle_submit_message($request);
            ...
            return new WP_Error('ch_error', $e->getMessage(), ['status' => 500]);
        }
    }

    public function handle_ch_sessions(WP_REST_Request $request) {
        try {
            $user_id = get_current_user_id();
            if (!$user_id) { return new WP_Error('ch_unauthorized', __('Nicht eingeloggt.', 'dual-chatbot'), ['status' => 401]); }
            global $wpdb;
            $sessions_table = $wpdb->prefix . 'chatbot_sessions';
            $prev = $wpdb->suppress_errors(true);
            $rows = $wpdb->get_results($wpdb->prepare(
                "SELECT session_id, COALESCE(title, '') AS title, updated_at FROM $sessions_table WHERE user_id = %d ORDER BY updated_at DESC LIMIT 100",
                $user_id
            ), ARRAY_A);
            $wpdb->suppress_errors($prev);
            $list = [];
            foreach (($rows ?: []) as $r) {
                $title = $r['title'];
                if ($title === '' || $title === null) { $title = 'Chat vom ' . mysql2date(get_option('date_format') . ' H:i', $r['updated_at']); }
                $list[] = [ 'session_id' => $r['session_id'], 'title' => $title, 'updated_at' => $r['updated_at'] ];
            }
            return rest_ensure_response(['ok' => true, 'sessions' => $list]);
        } catch (Throwable $e) {
            return rest_ensure_response(['ok' => true, 'sessions' => []]);
        }
    }

    public function handle_ch_context(WP_REST_Request $request) {
        $ctx = sanitize_key((string) $request->get_param('context'));
        $advisor = [
            'welcome' => get_option(Dual_Chatbot_Plugin::OPTION_GREETING_MEMBERS, ''),
            'model'   => get_option(Dual_Chatbot_Plugin::OPTION_ADVISOR_MODEL, 'gpt-4o'),
        ];
        $faq = [
            'welcome' => get_option(Dual_Chatbot_Plugin::OPTION_GREETING_FAQ, ''),
            'model'   => get_option(Dual_Chatbot_Plugin::OPTION_FAQ_MODEL, 'gpt-4o'),
        ];
        $map = [ 'advisor' => $advisor, 'members' => $advisor, 'faq' => $faq ];
        if (!isset($map[$ctx])) {
            return new WP_Error('ch_not_found', __('Unbekannter Kontext.', 'dual-chatbot'), ['status' => 404]);
        }
        return rest_ensure_response(['ok' => true, 'context' => $ctx, 'data' => $map[$ctx]]);
    }
