...
async function sendMessageWithExtras(text, containerEl, webSearch, options = {}) {
    ...
    try {
      ...
      // Fallback: non-streaming – replace, do not append
      try {
        const response = await wpFetch(`${restBase}/submit_message`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
function initChatWidget(openDirect = false) {
  ...
  async function initAdvisorView() {
    ...
    // Aufnahme-Aktionsleiste (Vorschau + Abbrechen + Übernehmen)
...
document.body.classList.add('dual-chatbot-modal-open');
      ...
      try {
        ...
        if (data.sessions && data.sessions.length) {
          data.sessions.forEach(sess => {
            ...
            const renameBtn = createElement('button', 'dual-chatbot-delete-btn', ['✎']);
  ...
  // Stronger normalization: ensure at most one bar per message across container
    ...
    try {
      ...
      msgs.forEach(msg => {
        // Remove any action bars from user messages entirely
        if (msg.classList.contains('dual-chatbot-user')) {
          msg.querySelectorAll('.dual-msg-actions').forEach(el => el.remove());
          return;
        }
        const bars = msg.querySelectorAll('.dual-msg-actions');
        if (bars.length > 1) {
          // Keep only the last one within the message
        ...
        // If missing, create lazily for bot messages
...
function appendMessageToContainer(containerEl, sender, text, id=null, opts={}) {
    ...
    // Observe the main chat for new assistant messages and attach actions (debounced)
      try {
        let normScheduled = false;
        const scheduleNormalize = () => {
          if (normScheduled) return;
          normScheduled = true;
        ...
        const actionsObserver = new MutationObserver(() => { scheduleNormalize(); });
        ...
        scheduleNormalize();
        setTimeout(scheduleNormalize, 150);
      } catch(_){}
